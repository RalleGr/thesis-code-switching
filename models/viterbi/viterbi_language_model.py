...
class ViterbiLanguageModel:
	...
	def dump(self, file_name=None):
		...
		print_status("Saved model at: %s" % file_name)
	...
	def load(cls, model_file, lex_file, lex_weight=1):
		...
		Args:
			model_file (str): ViterbiLanguageModel file name
			lex_file (str): Frequency lexicon file name
			lex_weight (float): Weight of the lexicon vs. the character model.
		"""
	...
	def load_lexicon(self, lex_file):
		...
		try:
			frequency_df = pd.read_csv(self.lex_file, encoding='utf-16')
		except:
			frequency_df = pd.read_csv(self.lex_file, encoding='utf-8')
		frequency_dict = frequency_df.set_index('word')['frequency'].to_dict()

		lex = {}
		total = 0
		for key in frequency_dict.keys():
			if len(str(key)) >= self.CASE_SENSITIVITY_THRESHOLD:
				word = key.lower()
			lex[key] = lex.get(key, lamb) + frequency_dict[key]
			total += frequency_dict[key]
	...
	def train(self, smooth_lambda=0.001):
		...
		n = self.n
		print_status("Training %d-gram model for language: %s" % (n, self.language))
		...
		for key in frequency_dict.keys():
			token = self.START + str(key) + self.END
			token_count = frequency_dict[key]
		...
		print_status("Model trained on %d tokens" % token_total)

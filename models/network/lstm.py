...
class LSTM:
    def __new__(self, inputChar_shape, inputSentence_shape, optimizer, embedding_type, main_output_lstm_units=64, dropout=0.2, recurrent_dropout_rate=0.2, learning_rate=0.01, momentum=0.9, return_optimizer=False):
        ...
        if ('fasttext' in embedding_type):
            char_bilstm_output = layers.LSTM(100, return_sequences=True, dropout=dropout, recurrent_dropout=recurrent_dropout_rate)(maskedInputChar)
        else:
            char_bilstm_output = layers.LSTM(768, return_sequences=True, dropout=dropout, recurrent_dropout=recurrent_dropout_rate)(maskedInputChar)

        # Combine the output of the two branches
        combined = layers.concatenate([char_bilstm_output, maskedInputSentence], axis=1)

        # Main output
        z = layers.LSTM(main_output_lstm_units, return_sequences=True, dropout=dropout, recurrent_dropout=recurrent_dropout_rate)(combined)

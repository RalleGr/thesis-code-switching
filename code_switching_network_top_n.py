...
arg_parser.add_argument("--nr_of_batches",type=int,help="Choose a number of batches [between 1 and 524]")
...
nr_of_batches = args.nr_of_batches
...
for s in encoded_sentences:
	...
	x_sentences.append((char_s, s_vectors_only, np.exp(s.sentence_cs), t_s, s.words))

# Sort sentences in descending order according to confidence scores
x_sentences.sort(key=lambda s: s[2], reverse=True)

with open('sentences_confidence_score.txt', 'w', encoding='utf-8') as f:
	for s in x_sentences[:200]:
		f.write(' '.join([w.word for w in s[4]]) + '\n') # tokens
	...
	j = 0
	for i in range(len(x_sentences) - 1, 0, -1):
		s = x_sentences[i]
		if (s[2] != 0.0 and j < 200):
			j += 1
			f.write(' '.join([w.word for w in s[4]]) + '\n') # tokens
			f.write(' '.join([str(i) for i in s[3]]) + '\n') # labels: 1 or 2
			f.write(str(s[2]) + '\n') # confidence score
...
# Use only the sentences with the highest confidence scores
x_sentences = x_sentences[:nr_of_batches * batch_size]
t_train = []
...
t_train = sequence.pad_sequences(t_train, maxlen=s_max_length + x_char_train.shape[1])
...
model_filepath = 'models/train_models/model_topn_' + lang1 + '_' + lang2 + '_' + architecture + '_' + str(units) + '_' + embedding_type + '_' + \
				optimizer + '_lr' + str(learning_rate) + '_ep' + str(epochs) + '_bsize' + str(batch_size) + '_mom' + str(momentum) + '_nrbatches' + str(nr_of_batches) + '.h5'
...
failed_instances_to_csv("top-n-instances", token_sentences, y, label_sentences)
